...
pub fn panel(doc: RwSignal<SimpleDoc>) -> impl View {
    ...
    scroll(view).on_event_cont(EventListener::PointerDown, move |x| {
        ...
        .on_scroll(move |viewport| {
            doc.update(|x| {
                x.update_viewport_by_scroll(viewport)
            });
        }).scroll_style(move |s| s.handle_thickness(handle_thickness).shrink_to_fit())
        .style(move |x| x.width(300.0).height(300.0).border(1.0)
...
impl View for EditorView {
    ...
    fn layout(
        ...
        cx.layout_node(self.id, true, |_cx| {
            ...
            let view_size = self.doc.with_untracked(|x| {
                x.view_size()
            });
