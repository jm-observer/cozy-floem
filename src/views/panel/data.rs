...
use floem::{
    ...
    prelude::{RwSignal, SignalGet, SignalUpdate, SignalWith},
...
impl SimpleDoc {
    ...
    pub fn pointer_down(
        ...
        match event.count {
            ...
            2 => {
                let offset = self.offset_of_pos(event.pos)?.0;
                let (start_code, end_code) = WordCursor::new(
                    &self.lines.line_info()?.0,
                    offset
                )
                .select_word();
                self.cursor.position = Position::Region {
