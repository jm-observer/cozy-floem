...
use floem::{
    ...
    reactive::{RwSignal, Trigger, create_trigger},
    text::{
        AttrsList, FONT_SYSTEM
    }
};
...
impl SimpleDoc {
    ...
    pub fn select_of_cursor(&self) -> Result<Vec<Rect>> {
        ...
        let (start_point, mut start_line, _) =
            self.point_of_offset(start_offset)?;
        let (mut end_point, end_line, _) =
    ...
    pub fn append_line(
        ...
        let points: Vec<(f64, f64, Hyperlink)> = hyperlink
            ...
            .collect();
        let hyperlinks: Vec<(Point, Point)> = points
            .iter()
            .map(|(x0, x1, _)| {
